"""Test: capture mug from ZED, send to cloud Hunyuan3D Full, align + visualize."""
...
def main():
    ...
    try:
        ...
        if image is None:
            return
        image.save(f"{data_dir}/mug_crop_hunyuan3d_cloud.png")
    ...
    # Step 2: Send to cloud Hunyuan3D Full via Modal
    print("\nSending to cloud Hunyuan3D Full (A100 80GB)...")
    buf = io.BytesIO()
    ...
    hunyuan_pts = np.array(result['points'], dtype=np.float32)
    np.save(f"{data_dir}/mug_hunyuan3d_cloud.npy", hunyuan_pts)

    # Step 4: Align to ZED partial cloud
    print("\nAligning Hunyuan3D -> camera frame...")
    aligned_pts = align_to_partial(hunyuan_pts, partial_pts)
    np.save(f"{data_dir}/mug_hunyuan3d_cloud_aligned.npy", aligned_pts)
    ...
    cv2.imshow("Hunyuan3D Cloud Input Image", display_bgr)

    # Window 2: Raw Hunyuan3D mesh (canonical frame)
    ...
    vis_mesh.create_window("Hunyuan3D Cloud Mesh (canonical)", width=600, height=600, left=450)
