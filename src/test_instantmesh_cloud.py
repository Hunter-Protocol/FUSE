"""Test: capture mug from ZED, send to cloud InstantMesh, align + visualize."""
...
def align_to_partial(mesh_pts, partial_pts):
    """Align InstantMesh canonical-frame points to ZED camera-frame partial cloud."""
...
def main():
    ...
    try:
        ...
        if image is None:
            return
        image.save(f"{data_dir}/mug_crop_instantmesh.png")
    ...
    # Step 2: Send to cloud InstantMesh via Modal
    print("\nSending to cloud InstantMesh (A100 80GB)...")
    buf = io.BytesIO()
    image.save(buf, format="PNG")
    image_bytes = buf.getvalue()

    # Look up the deployed Modal class
    InstantMeshModel = modal.Cls.from_name("fuse-instantmesh", "InstantMeshModel")
    ...
    instantmesh_pts = np.array(result['points'], dtype=np.float32)
    np.save(f"{data_dir}/mug_instantmesh_cloud.npy", instantmesh_pts)

    # Step 4: Align to ZED partial cloud
    print("\nAligning InstantMesh -> camera frame...")
    aligned_pts = align_to_partial(instantmesh_pts, partial_pts)
    np.save(f"{data_dir}/mug_instantmesh_aligned.npy", aligned_pts)
    ...
    cv2.imshow("InstantMesh Input Image", display_bgr)

    # Window 2: Raw InstantMesh mesh (canonical frame)
    ...
    vis_mesh.create_window("InstantMesh Mesh (canonical)", width=600, height=600, left=450)
    ...
    vis_pc.create_window("Aligned: RED=ZED, CYAN=InstantMesh", width=600, height=600, left=1100)
